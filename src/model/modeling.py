...
from transformers.generation import GenerationMixin  # type: ignore[attr-defined]
from transformers.integrations import use_kernel_forward_from_hub  # type: ignore[attr-defined]
...
from transformers.modeling_utils import (
    ...
    PreTrainedModel,
    sdpa_attention_forward,  # type: ignore[attr-defined]
)
from transformers.processing_utils import Unpack
from transformers.utils import (
    ModelOutput,  # type: ignore[attr-defined]
    TransformersKwargs,  # type: ignore[attr-defined]
    auto_docstring,  # type: ignore[attr-defined]
    can_return_tuple,  # type: ignore[attr-defined]
...
class RotaryEmbedding(nn.Module):
    def __init__(self, config: PaddleOCRVLConfig, device=None):
        ...
        self.config = config
        self.rope_init_fn = ROPE_INIT_FUNCTIONS[self.rope_type]  # type: ignore[index]

        inv_freq, self.attention_scaling = self.rope_init_fn(self.config, device)
        self.register_buffer("inv_freq", inv_freq, persistent=False)
        self.original_inv_freq = self.inv_freq  # type: ignore[assignment]
    ...
    def _dynamic_frequency_update(self, position_ids, device):
        ...
        if (
            ...
            self.register_buffer("inv_freq", self.original_inv_freq, persistent=False)  # type: ignore[arg-type]
...
def eager_attention_forward_ernie(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    key_states = repeat_kv(key, module.num_key_value_groups)  # type: ignore[attr-defined]
    value_states = repeat_kv(value, module.num_key_value_groups)  # type: ignore[attr-defined]
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        query_states, key_states = apply_multimodal_rotary_pos_emb(
            query_states, key_states, cos, sin, self.rope_scaling["mrope_section"]  # type: ignore[index]
        )
...
class Ernie4_5Model(Ernie4_5PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],  # type: ignore[override]
    ) -> BaseModelOutputWithPast:
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids)  # type: ignore[assignment]
        ...
        if cache_position is None:
            ...
            cache_position = torch.arange(  # type: ignore[assignment]
                past_seen_tokens,
                past_seen_tokens + inputs_embeds.shape[1],  # type: ignore[union-attr]
                device=inputs_embeds.device,  # type: ignore[union-attr]
            )

        if position_ids is None:
            position_ids = cache_position.view(1, 1, -1).expand(  # type: ignore[assignment]
                3, inputs_embeds.shape[0], -1  # type: ignore[union-attr]
            )
        elif position_ids.dim() == 2:
            position_ids = position_ids[None, ...].expand(3, position_ids.shape[0], -1)  # type: ignore[assignment]

        causal_mask = create_causal_mask(
            config=self.config,
            inputs_embeds=inputs_embeds,  # type: ignore[call-arg]
            attention_mask=attention_mask,
...
class Ernie4_5ForCausalLM(Ernie4_5PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        logits = self.lm_head(hidden_states[:, slice_indices, :])  # type: ignore[index]
...
class Projector(nn.Module):
    ...
    def forward(
        ...
        if isinstance(image_features, (list, tuple)):
            ...
            return processed_features  # type: ignore[return-value]

        dims = image_features.shape[:-1]
        dim = image_features.shape[-1]
        image_features = image_features.view(int(np.prod(dims)), dim)
...
class PaddleOCRVisionEmbeddings(nn.Module):
    ...
    def interpolate_pos_encoding(
        ...
        patch_pos_embed = patch_pos_embed.reshape(
            1, sqrt_num_positions, sqrt_num_positions, dim  # type: ignore[arg-type]
        )
    ...
    def fetch_position_embedding_lfu_cache(self, embeddings, h, w, max_cache=20):
        ...
        if len(self.cache_position_embedding) >= max_cache:
            min_hit_grid = min(
                self.cache_position_count, key=self.cache_position_count.get  # type: ignore[arg-type]
            )
    ...
    def forward(
        ...
        if pixel_values.dim() == 5:
            ...
            if interpolate_pos_encoding and image_grid_thw is not None:
                ...
                for image_grid in image_grid_thw:
                    t, h, w = image_grid
                    end = start + t * h * w  # type: ignore[operator]
                    image_embeddings = embeddings[start:end, :]
                    position_embedding = (
                        self.interpolate_pos_encoding(image_embeddings, h, w, True)  # type: ignore[arg-type]
                        .squeeze(0)
                        .repeat(t, 1)  # type: ignore[arg-type]
                    )
...
class PaddleOCRAttention(nn.Module):
    ...
    def forward(
        ...
        if not use_flash_attn:
            ...
            max_seqlen_q = (cu_seqlens[1:] - cu_seqlens[:-1]).max().item()  # type: ignore[index, operator]
            max_seqlen_k = (cu_seqlens[1:] - cu_seqlens[:-1]).max().item()  # type: ignore[index, operator]
            assert (
                cu_seqlens[-1].item()  # type: ignore[index]
                == queries.shape[0]
                == keys.shape[0]
                == values.shape[0]
            ), (cu_seqlens, queries.shape, keys.shape, values.shape)

            attn_output = flash_attn_varlen_func(  # type: ignore[operator]
                queries,
                keys,
                values,
                cu_seqlens,
...
class PaddleOCREncoder(nn.Module):
    ...
    def __init__(self, config: PaddleOCRVLConfig):
        ...
        self.layers = nn.ModuleList(
            [PaddleOCREncoderLayer(config) for _ in range(config.num_hidden_layers)]  # type: ignore[arg-type]
        )
    ...
    def forward(
        ...
        use_window_attn = window_size > 0 and vision_or_text == "vision"  # type: ignore[operator]
        ...
        reversed_window_indices: Optional[torch.Tensor] = None
        if use_rope is True:
            flatten_image_grid_thw = self.flatten_list(image_grid_thw)
            assert (
                sum([np.prod(x) for x in flatten_image_grid_thw])
        ...
        for encoder_layer in self.layers:
            if output_hidden_states:
                encoder_states = encoder_states + (  # type: ignore[operator]
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    encoder_layer.__call__,  # type: ignore[attr-defined]
                    hidden_states,
                    attention_mask,
                    output_attentions,
                    attn_cu_seqlens,
            ...
            if output_attentions:
                all_attentions = all_attentions + (layer_outputs[1],)  # type: ignore[operator]
...
class PaddleOCRVisionTransformer(nn.Module):
    def __init__(self, config: PaddleOCRVisionConfig):
        ...
        self.encoder = PaddleOCREncoder(config)  # type: ignore[arg-type]
    ...
    def forward(
        ...
        if return_pooler_output is True:
            if sample_indices is not None:
                ...
                if not vision_return_embed_list:
                    ...
                    padding_mask = list()  # type: ignore[assignment]
                    for idx, _state in enumerate(sample_hidden_state_list):
                        ...
                        padding_mask.append(mask)  # type: ignore[union-attr]
                    ...
                    padding_mask = (
                        torch.stack(padding_mask, dim=0)  # type: ignore[arg-type]
                        .float()
                ...
                return BaseModelOutputWithPooling(
                    last_hidden_state=sample_hidden_state,  # type: ignore[arg-type]
                    pooler_output=pooler_output,  # type: ignore[arg-type]
        ...
        for i in range(cu_seqlens.shape[0] - 1):  # type: ignore[union-attr]
            start = cu_seqlens[i]
            end = cu_seqlens[i + 1]
...
def apply_rotary_pos_emb_flashatt(
    ...
    q_embed = apply_rotary_emb(q.float(), cos.float(), sin.float()).type_as(q)  # type: ignore[operator]
    k_embed = apply_rotary_emb(k.float(), cos.float(), sin.float()).type_as(k)  # type: ignore[operator]
    return q_embed, k_embed
...
class SigLIPRotaryEmbedding(nn.Module):
    ...
    def forward(self, seqlen: int) -> torch.Tensor:
        seq = torch.arange(
            seqlen, device=self.inv_freq.device, dtype=self.inv_freq.dtype  # type: ignore[arg-type]
        )
        freqs = torch.outer(seq, self.inv_freq)  # type: ignore[arg-type]
        return freqs
...
class PaddleOCRVLForConditionalGeneration(Ernie4_5PreTrainedModel, GenerationMixin):
    ...
    def set_input_embeddings(self, value):
        self.model.embed_tokens = value  # type: ignore[assignment]
    ...
    def get_rope_index(
        ...
        if input_ids is not None and (
            ...
            for i, input_ids in enumerate(total_input_ids):  # type: ignore[assignment]
                input_ids = input_ids[attention_mask[i] == 1]  # type: ignore[index, assignment]
                image_nums, video_nums = 0, 0
                vision_start_indices = torch.argwhere(
                    input_ids == vision_start_token_id
                ).squeeze(1)
                vision_tokens = input_ids[vision_start_indices + 1]  # type: ignore[index]
                image_nums = (vision_tokens == image_token_id).sum()
                video_nums = (vision_tokens == video_token_id).sum()
                input_tokens = input_ids.tolist()  # type: ignore[union-attr]
                ...
                for _ in range(image_nums + video_nums):
                    ...
                    if ed_image < ed_video:
                        t, h, w = (
                            image_grid_thw[image_index][0],  # type: ignore[index]
                            image_grid_thw[image_index][1],  # type: ignore[index]
                            image_grid_thw[image_index][2],  # type: ignore[index]
                        )
                        second_per_grid_t = 0
                        image_index += 1
                        remain_images -= 1
                        ...
                        t, h, w = (
                            video_grid_thw[video_index][0],  # type: ignore[index]
                            video_grid_thw[video_index][1],  # type: ignore[index]
                            video_grid_thw[video_index][2],  # type: ignore[index]
                        )
            ...
            mrope_position_deltas = torch.tensor(
                mrope_position_deltas, device=input_ids.device  # type: ignore[union-attr]
            ).unsqueeze(1)
            ...
            if attention_mask is not None:
                ...
                position_ids = (
                    torch.arange(input_ids.shape[1], device=input_ids.device)  # type: ignore[union-attr]
                    .view(1, 1, -1)
                    .expand(3, input_ids.shape[0], -1)  # type: ignore[union-attr]
                )
                mrope_position_deltas = torch.zeros(
                    [input_ids.shape[0], 1],  # type: ignore[union-attr]
                    device=input_ids.device,  # type: ignore[union-attr]
                    dtype=input_ids.dtype,  # type: ignore[union-attr]
                )
        ...
        **kwargs,
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                pro = 0
                for idx, thw in enumerate(image_grid_thw):  # type: ignore[arg-type]
                ...
                mask_expanded = mask_unsqueezed.expand_as(inputs_embeds)  # type: ignore[arg-type]
                image_mask = mask_expanded.to(inputs_embeds.device)  # type: ignore[union-attr]

                image_embeds = image_embeds.to(
                    inputs_embeds.device, inputs_embeds.dtype  # type: ignore[union-attr]
                )

                inputs_embeds = inputs_embeds.masked_scatter(image_mask, image_embeds)  # type: ignore[union-attr, assignment]

            if attention_mask is not None:
                attention_mask = attention_mask.to(inputs_embeds.device)  # type: ignore[union-attr]
        # position_ids = None
        ...
        if position_ids is None and (
            ...
            if (
                ...
                or (past_key_values is None or past_key_values.get_seq_length() == 0)  # type: ignore[union-attr]
            ):
                position_ids, rope_deltas = self.get_rope_index(  # type: ignore[assignment]
                    input_ids,
                    image_grid_thw,
                    video_grid_thw,
                    second_per_grid_ts,
            ...
            # then use the prev pre-calculated rope-deltas to get the correct position ids
                ...
                batch_size, seq_length, _ = inputs_embeds.shape  # type: ignore[union-attr]
                delta = (
                    (cache_position[0] + self.rope_deltas).to(inputs_embeds.device)  # type: ignore[union-attr]
                    if cache_position is not None
                    else 0
                )
                position_ids = torch.arange(seq_length, device=inputs_embeds.device)  # type: ignore[union-attr, assignment]
                position_ids = position_ids.view(1, -1).expand(batch_size, -1)  # type: ignore[union-attr, assignment]
                if cache_position is not None:  # otherwise `deltas` is an int `0`
                    delta = delta.repeat_interleave(batch_size // delta.shape[0], dim=0)  # type: ignore[union-attr]
                position_ids = position_ids.add(delta)  # type: ignore[union-attr, assignment]
                position_ids = position_ids.unsqueeze(0).expand(3, -1, -1)  # type: ignore[union-attr, assignment]
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        _is_decode = (
            (_cp is not None and _cp[0] != 0)
            or (past_key_values is not None and getattr(past_key_values, "get_seq_length", lambda: 0)() > 0)
            or kwargs.get("next_sequence_length", None) == 1
        )
        if _is_decode:
    ...
    def _expand_inputs_for_generation(  # type: ignore[override]
        self,
        expand_size: int = 1,
        ...
        **model_kwargs,
        ...
        if expand_size == 1:
            return input_ids, model_kwargs  # type: ignore[return-value]
        ...
        def _expand_dict_for_generation_visual(dict_to_expand):
            ...
            for key in dict_to_expand:
                if key == "pixel_values":
                    ...
                    samples = torch.split(image_grid_thw, list(image_nums))  # type: ignore[arg-type]
                    ...
                    samples = torch.split(video_grid_thw, list(video_nums))  # type: ignore[arg-type]
                    lengths = [torch.prod(sample, dim=1).sum() for sample in samples]
                    dict_to_expand[key] = _repeat_interleave_samples(
                        dict_to_expand[key], lengths=lengths, repeat_times=expand_size
                    )
        ...
        if input_ids is not None:
            input_ids = input_ids.repeat_interleave(expand_size, dim=0)  # type: ignore[assignment]
